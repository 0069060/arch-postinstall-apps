...
ensure_multilib() {
  ...
  if ! confirm_system_config_edit "/etc/pacman.conf"; then
    announce_error "A edição de /etc/pacman.conf foi bloqueada ou cancelada."
    return 1
  fi

  backup_sensitive_system_file_if_exists "/etc/pacman.conf" "backup system config" || return 1
  ops_enable_multilib_config

  if ! multilib_enabled; then
    report_record_safety_operation "edit system config" "/etc/pacman.conf" "" "failed" "multilib não confirmado após edição"
    announce_error "Não foi possível habilitar multilib automaticamente."
    return 1
  fi
  report_record_safety_operation "edit system config" "/etc/pacman.conf" "" "succeeded" "multilib habilitado"
