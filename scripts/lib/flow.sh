...
# shellcheck source=scripts/lib/step-pipeline.sh
...
run_install() {
  ...
  if [[ "$execution_mode" == "check" || "$execution_mode" == "dry-run" ]]; then
    set_step_total "$(pipeline_count_steps_for_mode "$execution_mode")"
  fi
