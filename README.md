...
O projeto mantém o fluxo remoto `curl -fsSL https://obslove.dev | bash`, sincroniza o clone gerenciado em `~/Projects/arch-postinstall-apps`, instala os apps por categoria, prepara o ambiente, organiza backups soltos em `~/Backups`, garante clones auxiliares em `~/Projects`, `~/Repositories`, `~/Dots` e `~/EasyEffects-Preset`, e fecha a máquina com integração desktop, Codex CLI e GitHub SSH.
...
3. cria os diretórios principais usados pelo ambiente, move backups soltos em `~` para `~/Backups`, move repositórios Git soltos em `~` para `~/Repositories` e migra o clone gerenciado para `~/Projects/arch-postinstall-apps` quando necessário;
...
5. clona ou atualiza `~/EasyEffects-Preset`, `~/Projects/terminal-lyrics`, `~/Projects/synthetic-profile-generator`, `~/Dots/obslove`, `~/Repositories/dots-hyprland` e `~/Repositories/ii-vynx`, depois instala os apps da lista principal;
...
- `-c`, `--check`
  Valida o estado final esperado sem executar a instalação do runtime e sem autenticar `sudo` interativamente. Requisitos de root/sudo são reportados como requisito da execução real. Não é uma simulação completa da execução. No fluxo via `curl`, o bootstrap ainda pode sincronizar o clone local e preparar dependências iniciais se elas estiverem ausentes.
- `--dry-run`
  Mostra o plano local de execução sem aplicar alterações reais. Esse modo não autentica `sudo`, não instala pacotes, não move arquivos e não edita configurações.
- `--allow-home-changes`
  Permite explicitamente reorganizar backups ou repositórios soltos em `~` sem confirmação interativa. Use apenas em automações onde esse comportamento foi revisado.
- `--allow-system-config`
  Permite explicitamente editar configurações de sistema confirmáveis em modo não interativo, como `/etc/pacman.conf`.
- `-e`, `--exclusive-key`
  Destrutiva: remove as outras chaves SSH da conta no GitHub e mantém apenas a chave atual, mesmo que o GitHub SSH já esteja configurado. Essa opção exige digitar `DELETE GITHUB SSH KEYS` no terminal.
- `-n`, `--no-gh`
...
curl -fsSL https://obslove.dev | bash -s -- --dry-run
...
> [!NOTE]
> Operações surpreendentes em `~`, como mover backups ou repositórios Git soltos, mostram preview e exigem confirmação textual no terminal. Em modo não interativo, elas são bloqueadas por padrão.

## Modos de execução e segurança

O runtime separa três modos:

- `--check`: valida ambiente, configuração, artefatos e estado esperado sem aplicar alterações reais. Não autentica `sudo` interativamente. Requisitos de root/sudo aparecem no summary como requisito da execução real.
- `--dry-run`: monta o plano de execução local e renderiza operações planejadas. Não executa `pacman`, AUR, `systemctl`, `git` remoto, `gh`, `npm`, movimentações em `~` ou edições de shell rc.
- execução real: aplica as mudanças planejadas usando os wrappers internos, com status padronizado no summary.

Operações com side effect passam pelo operation planner sempre que possível. Cada operação registra tipo, alvo, comando planejado, risco, necessidade de root/sudo, confirmação textual, política não interativa, status, backup e orientação de rollback manual quando aplicável.

Confirmações perigosas exigem texto exato. Entradas vazias não são aceitas. Hoje os textos críticos são:
...
- `--allow-home-changes`: permite reorganização em `~` sem confirmação interativa.
- `--allow-system-config`: permite edição de configuração de sistema confirmável em modo não interativo.

Rollback automático completo ainda não existe. Quando há backup ou caminho reversível, o summary mostra o backup e/ou um `rollback=` com a orientação manual.

Limitações conhecidas:

- `--dry-run` não resolve dependências reais de `pacman`/AUR nem consulta o estado remoto de GitHub.
- autenticação interativa do `gh` continua delegada ao próprio GitHub CLI;
- rollback assistido é metadata e documentação, não execução automática.
